...
module.exports = {
  ...
  createPost: async function ({ postInputData }, req) {
    ...
    if (
      validator.isEmpty(
        postInputData.title || !validator.isLength(postInputData.title, { min: 5 })
      )
    ) {
    ...
    if (
      validator.isEmpty(
        postInputData.content || !validator.isLength(postInputData.content, { min: 5 })
      )
    ) {
  ...
  updatePost: async function ({ id, postInputData }, req) {
    if (!req.isAuth) {
      const error = new Error('Not authenticated');
      error.code = 401;
      throw error;
